...
function K:ApplyFont()
    local newFont = self.Shared:Fetch('font', self.db.profile.general.font)
    ...
    for i, FontObject in pairs(BlizFontObjects) do
        ...
        SetFont(FontObject, newFont, ForcedFontSize[i] or oldSize, oldStyle)
    end
    ...
    for i = 1, 50 do
        if _G["ChatFrame" .. i] then
            ...
            SetFont(_G["ChatFrame" .. i], newFont, ForcedFontSize[i] or oldSize, oldStyle)
        end
    end

    for _, barName in pairs(ActionBars) do
        for i = 1, 12 do
            if _G[barName .. "Button" .. i] then
                local _, oldSize, oldStyle  = _G[barName .. "Button"]:GetFont()

                print(barName .. "Button" .. i .. ', '.. test, oldSize, oldStyle)

                SetFont(_G[barName .. "Button" .. i], newFont, ForcedFontSize[i] or oldSize, oldStyle)
            else
                print(barName .. "Button" .. i .. ', not found')
            end
        end
    end
